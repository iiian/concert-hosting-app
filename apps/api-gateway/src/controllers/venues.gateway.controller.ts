import { Controller, Get, Param, Body, Post, Logger, Query } from '@nestjs/common';
...
export class VenuesController {
  ...
  @Get(`:${VID}/content/:${CID}`)
  async isAuthorized(@Param(`${CID}`) contentId, @Query(`is_user_authorized`) userId) {
  ...
  async getVenueContent(@Param(`${VID}`) venueId) {
    return (await this.contentServiceClient.getAllForVenue(venueId))
      .map(result => { delete result.uri; return result; });
  }
